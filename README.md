...
A **반응형 로또(KR 6/45)** 웹 애플리케이션으로, **Next.js (App Router)**, **Material UI v6**, **Supabase** 기반으로 구성되어 있습니다.  
예상 번호를 생성하고, 역대 당첨 번호와 비교하고, 로또 용지 패턴 보드를 분석하고, **여러 추출 모드**를 실행하고, **시뮬레이션 기반 모의추첨기**를 사용하고, 로또 용지 스타일로 출력하며, **DHLottery REST API** 기반의 공식 당첨 결과를 동기화할 수 있습니다. 또한 **Light / Dark / Night** 테마와 **ChatGPT 스타일 사이드바**를 제공합니다.
