            ...
            <Stack direction={{xs: 'column', sm: 'row'}} justifyContent="space-between" alignItems={{xs: 'stretch', sm: 'center'}} spacing={1}>
              <Stack spacing={0.25}>
                <Typography variant="subtitle2" sx={{fontWeight: 700}}>
                  {isEn ? 'AI Machine Learning Draw' : 'AI 머신러닝 추출'}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {isEn ? 'Random Forest predicts numbers from features such as frequency, odd-even ratio, and range distribution.' : 'Random Forest가 번호 빈도, 홀짝 비율, 번호대 구간 분포 같은 feature를 기반으로 확률이 높은 번호를 예측합니다.'}
                </Typography>
              </Stack>
              <Button
                variant="contained"
              ...
              >
                {warming ? (isEn ? 'Preparing model...' : '모델 준비 중...') : running ? (isEn ? 'Generating...' : '추출 중...') : (isEn ? 'AI Machine Learning Draw' : 'AI 머신러닝 추출')}
              </Button>
            </Stack>

            <Alert severity="info">
              {isEn ? 'The Random Forest model is prepared once, then the cached model is reused for instant prediction.' : '최초 1회만 Random Forest 모델을 준비하고, 이후에는 캐시된 모델로 즉시 예측합니다.'}
            </Alert>

            {warming && (
              <Alert severity="warning">
                {isEn ? 'The Random Forest model is being prepared in the background. The button will enable automatically when ready.' : 'Random Forest 모델을 백그라운드에서 준비 중입니다. 준비가 끝나면 버튼이 자동으로 활성화됩니다.'}
              </Alert>
            )}
            ...
            {result && (
              <Stack spacing={1.25}>
                ...
                <Stack direction={{xs: 'column', md: 'row'}} spacing={1.5}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {isEn ? `Trees ${result.prediction.metrics.estimators}` : `트리 ${result.prediction.metrics.estimators}개`}
                  </Typography>
                  <Typography variant="caption" color="text.secondary">
                    {isEn ? `Features ${result.prediction.metrics.featureCount}` : `feature ${result.prediction.metrics.featureCount}개`}
                  </Typography>
                </Stack>
