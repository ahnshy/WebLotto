            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'center' }} spacing={1}>
              <Stack spacing={0.25}>
                ...
                <Typography variant="body2" color="text.secondary">
                  LSTM이 회차 이력을 시계열로 학습해 다음 번호 조합을 예측합니다.
                </Typography>
              </Stack>
              <Button
                variant="contained"
              ...
              >
                {warming ? '모델 준비 중...' : running ? '추출 중...' : 'AI 딥러닝 추출'}
              </Button>
            </Stack>

            <Alert severity="info">
              최초 1회만 모델을 학습하고, 이후에는 캐시된 LSTM 모델로 바로 번호를 추출합니다.
            </Alert>

            {warming && (
              <Alert severity="warning">
                LSTM 모델을 백그라운드에서 준비 중입니다. 준비가 끝나면 버튼이 자동으로 활성화됩니다.
              </Alert>
            )}
            ...
            {result && (
              <Stack spacing={1.25}>
                ...
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {`소요 ${Math.round(result.prediction.metrics.elapsedMs / 100) / 10}s`}
                  </Typography>
                </Stack>
