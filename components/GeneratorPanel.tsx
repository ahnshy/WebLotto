...
export default function GeneratorPanel({ onPickAction }:{ onPickAction:(nums:number[])=>void }){
  ...
  return (<Card variant="outlined"><CardContent>
    ...
    <Stack direction="row" spacing={1} alignItems="center" sx={{ mt:1 }}>
      ...
      <Typography color="text.secondary">저장 및 우측 비교에 반영됩니다.</Typography>
    </Stack>
    {cur && (<Stack direction="row" sx={{mt:2, flexWrap:'nowrap', whiteSpace:'nowrap', overflowX:'auto'}}>{cur.map(n=><NumberBall key={n} n={n} size={40} mr={0.5}/>)}</Stack>)}
  </CardContent></Card>);
}
